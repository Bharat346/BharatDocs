...
const processHtmlContent = (html, theme, headingRefs = {}) => {
  ...
  headingElements.forEach((heading, index) => {
    ...
    if (headingRefs && !headingRefs.current) {
      headingRefs.current = {};
    }
    if (headingRefs && headingRefs.current) {
  ...
  const alertElements = doc.querySelectorAll(
    ".note, .tip, .warning, .caution, .danger, .error",
  );
...
export default function MDXContent({
  content, // HTML string from API
  theme = "dark",
  headingRefs = { current: {} },
}) {
  const [processedHtml, setProcessedHtml] = useState("");

  // Process HTML on client side (DOM operations)
  useEffect(() => {
    if (!content) return;
    const html = processHtmlContent(content, theme, headingRefs);
