...
export async function getPublishedBlogs(opts) {
  ...
  return globalCoalescer.execute(
    { route: 'getPublishedBlogs', queryParams: opts },
    () => cached("queries", key, () => _getPublishedBlogs(opts))
  );
}
...
export async function getBlogBySlug(slug) {
  return globalCoalescer.execute(
    { route: 'getBlogBySlug', queryParams: { slug } },
    () => cached("queries", `blog:${slug}`, () => _getBlogBySlug(slug))
  );
}
...
export async function getFeaturedBlogs(limit = 3) {
  return globalCoalescer.execute(
    { route: 'getFeaturedBlogs', queryParams: { limit } },
    () => cached("queries", `blogs:featured:${limit}`, () => _getFeaturedBlogs(limit))
  );
}
...
export async function getAllBlogTags() {
  return globalCoalescer.execute(
    { route: 'getAllBlogTags' },
    () => cached("tags", "blog-tags", _getAllBlogTags)
  );
}
