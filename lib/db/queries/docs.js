...
export async function getDocsByParentSlug(parentSlug, tagSlug) {
  ...
  return cached("queries", key, () => _getDocsByParentSlug(parentSlug, tagSlug));
}
...
export async function getAllDocs() {
  return cached("queries", "docs:all", _getAllDocs);
}
...
export async function getRecentDocs(limit = 6) {
  return cached("queries", `docs:recent:${limit}`, () => _getRecentDocs(limit));
}
