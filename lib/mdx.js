...
import { MDXComponents } from '@/components/DocsPage/shared/MDXComponents';
import { unstable_cache } from 'next/cache';
import { enhancedFetch, NETWORK_CONFIG } from './network/fetcher';
...
const fetchDocsBase = async (filePath) => {
  ...
  const opts = { headers, cacheTTL: NETWORK_CONFIG.CACHE_TTL * 24, retries: 2 };

  // Try primary URL
  let res = await enhancedFetch(url, opts);

  // Fallback logic
  if (res.status === 404 && fallbackUrl) {
    console.log(`[fetchDocsFromGithub] 404 on primary. Trying fallback URL: ${fallbackUrl}`);
    res = await enhancedFetch(fallbackUrl, opts);
