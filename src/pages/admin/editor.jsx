...
const TextEditor = () => {
  ...
  const convertToMDX = (html) => {
    try {
      ...
      // Process accordions before turndown
      const accordions = tempDiv.querySelectorAll('[data-component="accordion"]');
      accordions.forEach(accordion => {
        // Get the title from data attribute
        const title = accordion.getAttribute('data-title') || 'Accordion';
        
        // Get the content (should be the Accordion component's children)
        const content = accordion.innerHTML;
        
        // Replace with MDX format
        accordion.outerHTML = `<Accordion title="${title}">\n${content}\n</Accordion>`;
      });
  
      // Now convert to MDX using turndown
      const mdxContent = mdxConverter.turndown(tempDiv.innerHTML);
  ...
  const handleInsertAccordion = () => {
    ...
    // Save content
    saveContentToLocal();
    toast.success("Accordion inserted");
  };
  ...
  const handleEscape = () => {
    ...
    if (currentNode instanceof HTMLElement) {
      ...
      if (currentNode instanceof HTMLElement) {
        if (currentNode.closest('[data-type="accordion"]')) {
          const accordion = currentNode.closest('[data-type="accordion"]');
          const newLine = document.createElement("p");
          newLine.innerHTML = "<br>";
          accordion.after(newLine);
    
          range.selectNodeContents(newLine);
          range.collapse(true);
          toast.info("Exited accordion");
          return;
        }
      }
  ...
  return (
    <div className="flex flex-col h-full p-6 bg-background w-full max-w-4xl mx-auto mt-10">
